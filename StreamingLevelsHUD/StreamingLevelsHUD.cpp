...
static void RenderTextSLH(std::wstring msg, const float x, const float y, const char r, const char g, const char b, const float alpha, SDK::UCanvas* can)
{
...
static string hudFuncName = std::string("Function BIOC_Base.BioHUD.PostRender");
...
void __fastcall HookedPE(SDK::UObject* pObject, void* edx, SDK::UFunction* pFunction, void* pParms, void* pResult)
{
	...
	if (funcName.compare(hudFuncName) == 0)
	{
		...
		const auto hud = static_cast<SDK::ABioHUD*>(pObject);
		// Render mem usage
		...
		if (hud->WorldInfo)
		{
			...
			if (hud->WorldInfo->StreamingLevels.Any()) {
				...
				for (int i = 0; i < hud->WorldInfo->StreamingLevels.Num(); i++) {
					wstringstream ss;
					SDK::ULevelStreaming* sl = hud->WorldInfo->StreamingLevels[i];
					if (sl->bShouldBeLoaded || sl->bIsVisible) {
						unsigned char r = 255;
						unsigned char g = 255;
						unsigned char b = 255;

						ss << sl->PackageName.GetName().c_str();
						if (sl->PackageName.Index > 0)
						{
							ss << "_" << sl->PackageName.Index;
						}
						if (sl->bIsVisible)
						{
							ss << " Visible";
